namespace qmr
{
	...
	export class GameLoadManager extends BaseNotify
	{
		...
		public async loadLoginRes()
		{
			...
			await this.loadLoginBaseAnimation();
		...
		*/
		...
		private async loadLoadingViewRes(){
			return new Promise((resolve, reject) =>{
				...
				ResManager.getRes(GameLoadingView.getBgName(), comFunc, this, LoadPriority.IMMEDIATELY);
			})
		}
		...
		private async loadLoginBaseAnimation(){
			return new Promise((resolve, reject) =>{
				let resArr = [];
				//转圈加载特效模型
				resArr.push({ path: SystemPath.defaultPath, res: "168_idle" });
				let totalCount = resArr.length;
				let loadedCount = 0;
				let comFunc = function(){
					loadedCount ++;
					if(loadedCount >= totalCount){
